...
class TradeMonitor:
    ...
    async def add_trade_to_monitor(self, trade: OpenTrade):
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info(f"🔭 MONITORANDO TRADE: {symbol}")
            logger.info("=" * 60)
            logger.info(f"   ID: {trade.trade_id}")
            logger.info(f"   Entrada: ${trade.entry_price:.2f}")
            logger.info(f"   Target: ${trade.target_price:.2f} (+{self.config.TARGET_PROFIT_NET * 100:.2f}%)")
            logger.info(f"   Stop: ${trade.stop_loss_price:.2f} (-{self.config.STOP_LOSS_PERCENTAGE * 100:.2f}%)")
            logger.info(f"   Quantidade: {trade.quantity:.4f}")
        ...
        try:
            ...
            for trade in trades_to_check:
                ...
                if trade.should_take_profit(self.config.TARGET_PROFIT_NET):
                    logger.info("=" * 60)
                    logger.info(f"🎯 TAKE PROFIT ATINGIDO: {symbol}")
                    logger.info("=" * 60)
                    logger.info(f"   Entrada: ${trade.entry_price:.2f}")
                    logger.info(f"   Saída: ${current_price:.2f}")
                    logger.info(f"   PnL: {trade.pnl_percent * 100:+.2f}% (${trade.pnl_usdt:+.2f})")
                    logger.info("=" * 60)
                ...
                # VERIFICAR STOP LOSS
                    ...
                    logger.warning("=" * 60)
                    logger.warning(f"🛑 STOP LOSS ATINGIDO: {symbol}")
                    logger.warning("=" * 60)
                    logger.warning(f"   Entrada: ${trade.entry_price:.2f}")
                    logger.warning(f"   Saída: ${current_price:.2f}")
                    logger.warning(f"   PnL: {trade.pnl_percent * 100:+.2f}% (${trade.pnl_usdt:+.2f})")
                    logger.warning("=" * 60)
