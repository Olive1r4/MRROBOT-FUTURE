    ...
    async def process_entry_tick(self, symbol: str, current_price: float):
        ...
        try:
            ...
            if time_since_last >= 30:
                ...
                logger.info(f"{'='*60}")
                logger.info(f"   RSI: {state.rsi:.2f} {'✅' if condition_rsi else '❌'} (< {self.config.RSI_OVERSOLD})")
                logger.info(f"   BB Lower: ${state.bb_lower:.2f} {'✅' if condition_bb else '❌'} (preço <= banda)")
                logger.info(f"   EMA 200: ${state.ema_200:.2f} {'✅' if condition_ema else '❌'} (preço > EMA)")
                logger.info(f"{'='*60}")
            ...
            if condition_rsi and condition_bb and condition_ema:
                ...
                logger.info("=" * 60)
                logger.info(f"   ✅ RSI: {state.rsi:.2f} < {self.config.RSI_OVERSOLD}")
                logger.info(f"   ✅ Preço (${current_price:.2f}) <= BB Lower (${state.bb_lower:.2f})")
                logger.info(f"   ✅ Preço (${current_price:.2f}) > EMA 200 (${state.ema_200:.2f})")
                logger.info("=" * 60)
